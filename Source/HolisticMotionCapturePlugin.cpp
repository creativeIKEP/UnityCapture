...
struct HolisticMotionCaptureInstance
{
...
extern "C" __declspec(dllexport) HolisticMotionCaptureInstance* CaptureCreateInstance(int CapNum)
{
	HolisticMotionCaptureInstance* c = new HolisticMotionCaptureInstance();
	memset(c, 0, sizeof(HolisticMotionCaptureInstance));
	c->Sender = new SharedImageMemory(CapNum);
	return c;
}

extern "C" __declspec(dllexport) void CaptureDeleteInstance(HolisticMotionCaptureInstance* c)
{
	if (!c) return;
	delete c->Sender;
	if (c->Textures)
...
extern "C" __declspec(dllexport) int CaptureSendTexture(HolisticMotionCaptureInstance* c, void* TextureNativePtr, int Timeout, unsigned int ExtraBuffers, SharedImageMemory::EResizeMode ResizeMode, SharedImageMemory::EMirrorMode MirrorMode, bool IsLinearColorSpace)
{
