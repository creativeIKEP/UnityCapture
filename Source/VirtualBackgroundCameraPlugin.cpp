...
struct VirtualBackgroundCameraInstance
{
...
extern "C" __declspec(dllexport) VirtualBackgroundCameraInstance* CaptureCreateInstance(int CapNum)
{
	VirtualBackgroundCameraInstance* c = new VirtualBackgroundCameraInstance();
	memset(c, 0, sizeof(VirtualBackgroundCameraInstance));
	c->Sender = new SharedImageMemory(CapNum);
	return c;
}

extern "C" __declspec(dllexport) void CaptureDeleteInstance(VirtualBackgroundCameraInstance* c)
{
	if (!c) return;
	delete c->Sender;
	if (c->Textures)
...
extern "C" __declspec(dllexport) int CaptureSendTexture(VirtualBackgroundCameraInstance* c, void* TextureNativePtr, int Timeout, unsigned int ExtraBuffers, SharedImageMemory::EResizeMode ResizeMode, SharedImageMemory::EMirrorMode MirrorMode, bool IsLinearColorSpace)
{
